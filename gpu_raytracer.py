...
# Convert the resulting_color to NumPy array for image processing
resulting_color_np = np.asarray(resulting_color.components())

# Convert to uint8 and reshape for image display
image_array = [Image.fromarray((255 * np.clip(c, 0, 1)).astype(np.uint8).reshape((height, width)), "L") for c in resulting_color_np]
